...
def main():
    ...
    if rank >=  0: ## dummpy if statement, could be changed for other uses
        ...
        input_tensor_shape = (20,64,64,64)
        input_image = np.random.randn(input_tensor_shape[0],input_tensor_shape[1],input_tensor_shape[2],input_tensor_shape[3])
        input_image = torch.FloatTensor(input_image)
        input_image = input_image.unsqueeze(1)
        print('data loaded!')

#        model = model_cnn_only.ModelParallelCNN(device_list)    # max: epoch=4,
        model = model_cnn_only.PipelineCNN(device_list, split_size=input_tensor_shape[0]//2)    # max: epoch=4,
